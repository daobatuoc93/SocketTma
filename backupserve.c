...
{
    const char *clientsay = "";
    struct Node *listClient = newAddr;
    int n;
    char buffer[1024] = {0};
    while (1)
    {
        ...
        sleep(1);
        n = read(listClient->client.sock, buffer, 1024);
        if (n < 0)
        {
        ...
        else if (n == 0)
        {
            ...
            close(listClient->sock);
        }
        else
        {
            struct Node *sendClient = head;
            printf("User [%s] was said: %s !\n", listClient->client.username, buffer);
            while (sendClient != NULL)
            {
                if (sendClient != listClient)
                {
                    if (write(sendClient->client.sock, buffer, sizeof(buffer)) > 0)
                    {
